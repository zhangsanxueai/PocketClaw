...
docker info >nul 2>&1
if %errorlevel% neq 0 (
    :: 尝试启动 Docker Desktop（如果进程不在运行则启动，避免重复启动造成 backend.lock 冲突）
    ...
    if %errorlevel% neq 0 (
        ...
        echo [提示] Docker Desktop 进程已启动但尚未完全就绪，等待中...
    )
    echo        等待 Docker 就绪中（最多 60 秒）...
    set "WAIT=0"
    :wait_docker
    if !WAIT! geq 60 (
...
if defined FREE_MB (
    if !FREE_MB! lss %MIN_DISK_MB% (
        echo [错误] 磁盘可用空间不足 %MIN_DISK_MB% MB（当前: !FREE_MB! MB），安装可能失败！
    ) else (
        echo [成功] 磁盘空间充足。
    )
) else (
    echo [信息] 无法获取磁盘空间，继续执行...
)
...
echo [成功] 镜像已存在！如需重新加载: start.bat --reload
goto :check_env
...
if !ELAPSED! geq 600 (
    echo.
    echo [错误] 加载超时（已超过 10 分钟）！
    echo        可能原因: 文件损坏、Docker 资源不足。
    echo        请尝试: 重启 Docker Desktop 后重新运行本脚本。
...
echo        请在打开的记事本中填写 API Key，保存后关闭记事本继续。
notepad "%ENV_FILE%"
goto :end
...
if exist "!CFG_DIR!\openclaw.json" (
    echo [成功] 已检测到配置文件，跳过初始化步骤。
    goto :start_services
)

echo [信息] 首次运行，开始执行初始化流程（约 6 个步骤）...
...
:: 6.2 生成 Gateway Token（优先使用 PowerShell 加密学随机）
echo.
...
if "!NEW_TOKEN!"=="" (
    :: 备用方案：时间戳 + 多个 RANDOM 拼接（注意 RANDOM 范围有限，易碰撞）
...
:: 6.3 更新 .env（先过滤掉旧值，再追加新值，避免修改其他行）
echo.
...
>> "%ENV_FILE%" (
    ...
    echo OPENCLAW_ALLOW_INSECURE_PRIVATE_WS=true
    echo OPENCLAW_TZ=
)
echo [成功] .env 已更新完成！
...
:: 6.5 写入配置文件，确保首次启动式正常运行
echo.
echo [6.5] 正在写入初始配置文件（确保首次启动正常运行）...
set "OC_JSON=!CFG_DIR!\openclaw.json"
powershell -NoProfile -Command "Set-Content -Path '!OC_JSON!' -Value '{\"gateway\":{\"mode\":\"local\",\"bind\":\"lan\",\"controlUi\":{\"dangerouslyDisableDeviceAuth\":true}}}' -Encoding UTF8" 2>nul
if not exist "!OC_JSON!" (
    (echo {"gateway":{"mode":"local","bind":"lan","controlUi":{"dangerouslyDisableDeviceAuth":true}}})>"!OC_JSON!" 2>nul
)
if not exist "!OC_JSON!" (
...
docker compose -f "%COMPOSE_FILE%" run --rm openclaw-cli config set gateway.controlUi.dangerouslyDisableDeviceAuth true >nul 2>&1
if %errorlevel% neq 0 (
    echo [警告] 设置 dangerouslyDisableDeviceAuth 失败，继续执行...
)
echo [成功] gateway 已固定完成！

echo.
echo [提示] 如需进一步配置（如 AI 提供商、频道等），请运行：
echo        - AI Provider:  openclaw onboard --auth-choice openai
echo        - 频道配置:     openclaw channels add --channel telegram --token ^<TOKEN^>
echo        - 引导配置:     openclaw onboard
echo        以上命令需通过 docker compose run 执行。
...
if not "!ENV_CUR_IMAGE!"=="%IMAGE_NAME%" (
    ...
    move /y "!ENV_TMP!" "%ENV_FILE%" >nul
    echo [信息] 自动更新 OPENCLAW_IMAGE 为 %IMAGE_NAME%
)
...
if %errorlevel% neq 0 (
    echo.
    echo [错误] 启动失败，请查看上方详细日志！
    echo        可能原因:
    echo          - 端口 18789 被占用（用 netstat -ano ^| findstr :18789 查看）
    echo          - docker-compose.yml 文件错误
    echo          - .env 参数不正确
    goto :fail
)

:: 等待服务健康检查（curl 在 Windows 上用 PowerShell 兜底）
echo [信息] 等待服务就绪...
set "HEALTH_WAIT=0"
:health_loop
if !HEALTH_WAIT! geq 30 (
    echo [警告] 服务启动超时，请手动访问 %APP_URL%
    goto :open_browser
)
timeout /t 2 /nobreak >nul
set /a HEALTH_WAIT+=2

:: 优先用 curl，不可用时用 PowerShell 兜底
...
:: ========== 自动打开带 Token 的浏览器（避免 pairing required）==========
:open_browser
echo.
echo ========================================
...
if not "!CURRENT_TOKEN!"=="" (
    ...
    start "" "!DASHBOARD_URL!"
    echo [成功] 已用带 Token 的 URL 打开浏览器！
    echo        如未自动打开，请手动访问:
    echo        !DASHBOARD_URL!
) else (
    start "" "%APP_URL%"
    echo [信息] 未获取到 Token，已打开浏览器，请手动输入 Token 配对。
)
...
curl -s -o nul -w "HTTP状态: %%{http_code}" %APP_URL% 2>nul && echo  - 服务可访问 || (
    powershell -NoProfile -Command "try{$r=(Invoke-WebRequest -Uri '%APP_URL%' -TimeoutSec 3 -UseBasicParsing).StatusCode;Write-Host 'HTTP状态:' $r '- 服务可访问'}catch{Write-Host '服务不可访问'}" 2>nul
)
echo.
goto :end
...
echo.
echo   (无参数)      首次运行 OpenClaw（首次运行将执行初始化流程）
